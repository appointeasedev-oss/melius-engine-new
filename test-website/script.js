  ...
  quizSubmit.addEventListener('click', function() {
    ...
    quizResults.innerHTML = `
      <div class="quiz-results-content">
        <h3>Quiz Results</h3>
        <p>Your Score: ${score}/3 (${percentage}%)</p>
        <p>${score === 3 ? 'Excellent! You\'re a black hole expert!' : score === 2 ? 'Good job! Keep learning.' : 'Keep studying! Try again.'}</p>
      </div>
    `;
    ...
    setTimeout(() => {
      ...
      quizResults.style.transition = 'opacity 0.5s';
    }, 100);
  });
}

// Black Hole 3D Model functionality
const modelViewport = document.getElementById('model-viewport');
const modelBtns = document.querySelectorAll('.model-btn');

if (modelViewport && modelBtns.length > 0) {
  let rotationAngle = 0;
  let zoomLevel = 1;
  ...
  modelBtns.forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      switch (action) {
        case 'rotate':
          rotationAngle += 45;
          modelViewport.style.transform = `rotateY(${rotationAngle}deg) scale(${zoomLevel})`;
          break;
        case 'zoom':
          zoomLevel += 0.1;
          modelViewport.style.transform = `rotateY(${rotationAngle}deg) scale(${zoomLevel})`;
        ...
        case 'reset':
          rotationAngle = 0;
          zoomLevel = 1;
          modelViewport.style.transform = 'rotateY(0) scale(1)';
          break;
      }
    });
  });
...
// Enhanced animations for timeline
...
// Enhanced animations for quiz
function animateQuiz() {
  const quizContainer = document.querySelector('.quiz-container');
  
  if (quizContainer) {
    quizContainer.style.animation = 'fadeInUp 0.8s ease-out';
    quizContainer.style.animationDelay = '0s';
  }
}

// Enhanced animations for 3D model
function animateModel() {
  const modelContainer = document.querySelector('.model-container');
  
  if (modelContainer) {
    modelContainer.style.animation = 'fadeInUp 0.8s ease-out';
    modelContainer.style.animationDelay = '0s';
  }
}
